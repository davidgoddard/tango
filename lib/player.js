...
export class Player {
    ...
    */
        ...
        if (this.current?.player) {
            // if (this.current?.player.seek() > 0) {
                ...
                this.options.progress({
                    track,
                    pos,
                    display: `${this.current.state} ${this.current.file.name} ${to_time(pos)}/${to_time(track.end - track.start)}`,
                    state: newState
                });
            // }
        } else {
    ...
    async loadNext() {
        ...
        const { track, silence } = this.options.nextUp(N);
        if (track) {
            const { player, url, file } = await this.createPlayer(track);
    ...
    async createPlayer(track) {
        ...
        player.once('load', function () {
            ...
            const reduction = Player.dBtoLinear(track.gain - this.options.systemGain)
            player.volume(reduction);

        }.bind(this));


        return { player, url, file }
