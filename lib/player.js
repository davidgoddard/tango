    ...
    */
    ...
    reportProgress(newState) {
        ...
        if (this.current?.player) {
            ...
            const player = this.current.player;
            const track = this.current.track;
            let pos = this.current.state == 'Stopped' ? 0 : Math.min(player.seek() * 1000 - track.metadata.start, 1000 * (track.metadata.end - track.metadata.start))
            this.options.progress({
                track,
                pos,
                display: `${this.current.state}: ${this.current.displayName}  ( ${to_time(pos)}/${to_time(1000 * (track.metadata.end - track.metadata.start))} )`,
                state: newState
            });
            // }
        } else {
        ...
        if (track) {
            ...
            if (player) {
                ...
                const next = {
                    ...
                    url,
                    unload: null,
                    gainReduction: ((this.systemGain - (track.metadata.meanVolume - track.metadata.maxVolume))),
                    displayName: (track.metadata.tags.find(tag => tag.name == 'title')?.value || track.fileHandle.name) + ' / ' + 
                    (track.metadata.tags.find(tag => tag.name == 'artist')?.value)
                }
